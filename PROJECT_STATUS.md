...
Status: **Active — Rosemary water-preference bounded research + source capture closed (docs local; unpushed); Lavender first real batch production-verified and closed (pushed `586bf8f` / impl `4a5f3ad`); repeatability planning complete (`READY_FOR_SECOND_REAL_BATCH_BOUNDED_RESEARCH`); no Rosemary artifacts yet; second-batch implementation planning next; automation deferred until Rosemary production verification; blueberry parent remains blocked; all 23 conflicts remain blocked; matcher still blocked; GOS paused; foundation Registry defaults remain realPacketCount/realRecordCount/realProfileCount = 0 — inert Lavender reviewed-data batch under explicit developer load only; no catalog schema / product consumer / eligibility authority**
Priority: High
Scope: Filter **UI and logic are not enabled yet**. Schema foundation for filter taxonomy fields is additive only. **Completed track:** … → Lavender Source Reference Capture Closure (pushed `7c9fed5`) → **First Real Lavender Reviewed-Data Batch Implementation** (pushed `4a5f3ad`) → **Lavender production verification** (pushed `586bf8f`) → Second Real Batch Repeatability Planning (read-only; `READY_FOR_SECOND_REAL_BATCH_BOUNDED_RESEARCH`) → **Rosemary Water Preference Bounded Research Closure** (this docs checkpoint). Catalog task **B** is **Frozen (v1)**. All **23** conflicted identities remain globally blocked; **36/36/36** remains legacy eligibility only. Foundation empty Registries remain **0 / 0 / 0**. Explicit developer-loaded Lavender batch counts **2 / 1 / 1**; planned future Rosemary batch counts **2 / 1 / 1** (not created). Batch validator remains reusable without module amendment; separate Rosemary explicit-run harness preferred later. **No** `index.html` import; **no** product consumer; **no** `needsReview`/eligibility/scalar/matcher/UI/GOS/v1b change. Legacy `sunNeeds`/`waterNeeds` remain Layer A compatibility-only. **GOS synthetic stack paused**. Matchers/UI not authorized.
...
**Next checkpoint:** Second Real Rosemary Batch Implementation Planning (separately gated; not begun here).
...
**Verdict identity:** `READY_FOR_SECOND_REAL_ROSEMARY_BATCH_IMPLEMENTATION_PLANNING`.
**Upstream:** Lavender production-verified (`586bf8f` / impl `4a5f3ad`); repeatability planning verdict `READY_FOR_SECOND_REAL_BATCH_BOUNDED_RESEARCH`.

External research and exact source-reference capture are **complete** for this narrow question. This checkpoint **records the result only**. It creates **no** second reviewed-data directory, JSON, manifest, Evidence Packet, Field Review record, Structured Climate Profile, validator, harness, loader, or automation agent. It changes **no** Lavender artifact, **no** batch validator module, and **no** product authority. Implementation planning remains separately gated and is **not** begun here.
...
**Done (local; unpushed):** Second Real Reviewed-Data Batch — Rosemary Water Preference Bounded Research Closure — *Salvia rosmarinus*; UC ANR + NC State exact references; mature outdoor-ground `water` / `preference` / `low` hypothesis; low-to-medium limitation; preference-versus-tolerance boundary; planned **2 / 1 / 1**; no artifacts; verdict `READY_FOR_SECOND_REAL_ROSEMARY_BATCH_IMPLEMENTATION_PLANNING`.

**Next (separately gated; not begun):** Second Real Rosemary Batch Implementation Planning. UI remains blocked. GOS remains paused. Blueberry remains excluded. Lavender batch remains developer-only and inert. No Rosemary artifacts yet. Automation remains deferred until Rosemary production verification. All **23** conflicts remain blocked. Do **not** clear needsReview; edit catalog/schema/`index.html`; grant product eligibility; implement Blueberry children; implement sun/water matchers; or enable filter UI without separate approval.
