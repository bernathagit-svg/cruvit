...
**Catalog task B — canonical plant identities: review and separately approve the real nine-ID `plantId` allocation pilot (`-Apply`).** Done so far: Canonical Identity Audit; **data-only Plant Identity Registry Foundation** (`data/plant-identity.registry.json` + `data/plant-identity.schema.json`); **Stable Plant ID / Dual-Key Migration Audit** (read-only); **Stable Plant ID schema foundation** — optional opaque `plantId` slot (`^plt_[a-z0-9]{16}$`) added to `CanonicalIdentity` with policy, **no values assigned**; **Plant ID Allocation Planning Audit** (read-only) with **Model C approved** (registry = authoritative `plantId`/`canonicalSlug`/alias layer, catalog = descriptive layer); **cross-entry validator** `scripts/validate-plant-identity.ps1` (read-only; 29 checks PASS; enforces global/registry-wide uniqueness, quarantine, referential consistency, and full canonical coverage); **registry expanded to full canonical coverage** (74 canonical entries + 3 pending conflicts, data only, **no `plantId` values**); and the **allocation tooling foundation** (`scripts/allocate-plant-ids.ps1`, dry-run-only; validator extended to 35 checks incl. eligibility/placement + optional `-ExpectedIdSlugs`; nine-member dry run + negative tests green; registry hash unchanged; `registryVersion` still `1.1.0`). The registry is **not wired into runtime** and same-slug duplicates remain **documented, not resolved**. The next identity step is to **review and separately approve the real nine-ID pilot allocation** — run `scripts/allocate-plant-ids.ps1 -Apply` for the approved nine slugs, then bump `registryVersion`→`1.2.0` and re-validate with `-ExpectedIdSlugs`. Dual-key adoption and duplicate resolution are later tasks (`avocado`/`strawberry-guava`/`mulberry` stay ineligible until explicit duplicate-resolution decisions; the 16 genus-level/Various identities stay `needsReview:true` and are excluded from the first pilot). Keep `profileSlug`/`meta.slug` for saved My Garden plants. Do not rename, merge, or delete records, do not modify saved user plants, and do not populate SR taxonomy arrays or implement filter UI in these identity steps. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
