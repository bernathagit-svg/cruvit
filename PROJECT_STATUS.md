...
| **B** | Canonical plant identities, aliases, and duplicate records | **Frozen (v1)** — **Canonical Identity Foundation v1** is frozen and production-verified (`registryVersion` **1.5.0**; **76** canonical; **1** pending = mulberry; **9** plantIds). Canonical identity remains **advisory**; legacy runtime remains **authoritative**; **no** `plantId`/`canonicalSlug` persistence. **My Garden canonical read-only adoption Phase A** (developer-only `CruvitGardenIdentityAudit`) is **production verified** at `5ad556b` — disabled by default; advisory/in-memory only; no runtime consumer; **no Sidecar approved**. Next: controlled coverage measurement on isolated test/demo fixtures → go/no-go for an in-memory read-only Sidecar. Climate accuracy C–E remain separate. **Later (locked, not started):** Growth Outcome Suitability forensic audit/pilot after canonical read-only adoption is safely completed. |)
...
**Status:** Locked later roadmap requirement — **documented only**. Do **not** design or implement in the current Phase A / Sidecar measurement work. Do **not** begin this layer until My Garden canonical read-only identity adoption is safely completed.
...
## High
- Controlled canonical identity coverage measurement on isolated test/demo fixtures → Sidecar go/no-go (Phase A production verified; Sidecar not approved yet)
...
**Controlled canonical identity coverage measurement using isolated test and demo garden fixtures, followed by a go/no-go decision for an in-memory read-only Sidecar.** Measure only via explicit developer `enable()` + `run()` on **isolated test/demo fixtures** (do **not** run against the user’s real stored garden). Evaluate: resolution coverage; pending-conflict rate; ambiguous/conflicting-signal rate; provisional/unresolved rate; migration-candidate rate; duplicate-instance behavior; mutation and storage safety; execution time across small, medium, and maximum-size fixture gardens. The Sidecar may be approved **only if** measurements show safe coverage, no mutation, acceptable conflict behavior, and negligible performance impact. Keep `CruvitGardenIdentityAudit` developer-only and disabled by default; results remain advisory/in-memory; legacy My Garden remains authoritative; **no** `plantId`/`canonicalSlug` persistence; **no** migration. Do not mix Live Search or **Growth Outcome Suitability** (locked later) into this measurement. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
