...
| **B** | Canonical plant identities, aliases, and duplicate records | **Frozen (v1)** — **Canonical Identity Foundation v1** is frozen and production-verified (`registryVersion` **1.5.0**; **76** canonical; **1** pending = mulberry; **9** plantIds). Canonical identity remains **advisory**; legacy runtime remains **authoritative**; **no** `plantId`/`canonicalSlug` persistence. **My Garden Phase A–C2 production verified** at `3b8452d` — inert Sidecar stopped by default; explicit developer consult only; **no runtime consumer / auto-population / persistence / UI / authority**. **Lazy/idle Sidecar population deferred** (no approved product consumer yet). Climate accuracy C–E remain separate. **Next roadmap investigation:** Growth Outcome Suitability forensic audit and architecture/data planning (investigation only — no engine/schema/catalog/UI/ranking implementation). Canonical persistence, Shared Plant Picker, and consumer integration remain separate explicitly approved phases. |)
...
**Other backlog:** confidence-aware scoring refinements; Product/Care Schedule runtime; **Smart Recommendations filter-based UX** (locked — schema foundation only so far; chat and results table unchanged; sun/water logic/UI not started); **Growth Outcome Suitability** (next recommended task = forensic audit + architecture/data planning only; no engine/schema/catalog/UI/ranking implementation until separately approved).
...
| **17** | Growth Outcome Suitability | **Next — forensic audit / architecture planning only** (after Phase A–C2 production verified). No engine/schema/catalog/UI/ranking implementation until separately approved. Before broad Smart Recommendations or Add Plant suitability expansion. |
...
- **17 — Growth Outcome Suitability:** next recommended investigation — forensic audit + architecture/data planning only (see dedicated plan). Do **not** implement engine/schema/catalog/UI/ranking until separately approved.

---

# Growth Outcome Suitability Plan

**Status:** Next recommended investigation — **forensic audit and architecture/data planning only**. Do **not** implement an engine, change schemas, rewrite catalog data, change UI, or change Smart Recommendations / Add Plant ranking in the next audit task. Canonical read-only identity adoption (Phase A–C2) is production verified; Sidecar remains inert infrastructure.
...
## High
- Read-only Growth Outcome Suitability forensic audit and architecture/data planning (Phase C2 production verified; Sidecar lazy/idle population deferred; investigation only — no engine/schema/catalog/UI/ranking change)
...
- Growth Outcome Suitability — **next investigation**; forensic audit + architecture/data planning only (Phase A–C2 production verified; Sidecar remains inert). No engine/schema/catalog/UI/ranking implementation yet. Before broad Smart Recommendations / Add Plant suitability expansion. See dedicated plan.
...
**Read-only Growth Outcome Suitability forensic audit and architecture/data planning.** Investigate how CRUVIT can distinguish survival, vegetative growth, flowering, fruit set and fruit ripening/yield, long-term thriving and reliability, user goal and recommended use, plus location/climate/microclimate, container versus ground, cultivar effects, confidence, evidence sources, regional applicability, and unknown fallback. This task is **investigation and planning first**: **no** engine implementation; **no** schema change; **no** catalog-wide data change; **no** UI change; **no** Smart Recommendations ranking change; **no** Add Plant behavior change. Sidecar automatic lazy/idle population remains **deferred** (no approved product consumer yet). Canonical persistence, Shared Plant Picker, and consumer integration remain separate explicitly approved phases. Keep canonical identity advisory and legacy My Garden authoritative. Do not mix Live Search or Sidecar consumer wiring into this audit. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
