...
|------------|--------|-------|
| **My Garden Canonical Read-Only Adoption — Phase C1 complete locally** | Done (local) | **Behavior-preserving internal extraction only; no Sidecar, cache, Map, start/consult hooks, consumers, persistence, migration, UI/task use, plant mutation, authority change, or `index.html` change.** Extracted verified GardenPlant signal collection + canonical-result consolidation into shared pure module `modules/identity/garden-plant-identity-consult.js`, loaded via **ES-module import** from `my-garden-identity-audit.js` (no classic-script conversion; **`index.html` unchanged**). Frozen global `window.CruvitGardenPlantIdentityConsult` exposes: `collectGardenPlantIdentitySignals`, `resolveGardenPlantIdentityReadOnly`, `buildGardenPlantIdentityFingerprintInput`, `GARDEN_IDENTITY_SIGNAL_FIELDS`. Pure contract: no automatic execution, storage, DOM, network, timers, console, or second resolver/registry fetch; Shadow passed explicitly; deeply frozen results; plants/meta never mutated. Signal collection + consolidation are now one shared source of truth; the developer audit remains only a reporting/orchestration layer (`enable`/`disable`/`run`/`runStored`/`latest`/`snapshot`/`clear`/`status`; disabled by default; max 10 reports / 500 plants). Pending conflicts and conflicting canonical signals remain quarantined (`pending_conflict` / `ambiguous`+`conflicting_signals`; null `plantId`/`canonicalSlug`). **No Sidecar, cache, persistence, migration, UI/task consumer, or authority change exists.** Verified: audit harness **126/126 PASS**; resolver **108/108**; reconciliation **158 / 156 PASS / 2 EXPECTED_QUARANTINED_CONFLICT / 0 unexpected**; validator **81 PASS** exit 0; network deltas fetch/XHR/beacon/WebSocket/registry all **0**; preliminary performance medians ~**0.70 / 2.5 / 14.0 ms** for 20 / 100 / 500 plants; `registryVersion` **1.5.0**; **76** canonical; **1** pending; **9** plantIds; `git diff --check` clean. **Rollback** = restore prior private consult functions into the audit module, delete the shared consult file, and revert harness/docs. **Next recommended task:** Phase C2 — smallest inert developer-only in-memory Sidecar (see Next Recommended Task). **Growth Outcome Suitability** remains locked after safe canonical read-only adoption. |
...
## High
- Phase C2 — smallest inert developer-only in-memory My Garden canonical identity Sidecar (Phase C1 shared consult complete; Sidecar stopped by default; explicit consult only; no consumers / auto-population / persistence / UI / authority)
...
**Phase C2 — implement the smallest inert developer-only in-memory My Garden canonical identity Sidecar, stopped by default and explicit consultation only.** C2 authorizes only: an inert in-memory module; stopped on every fresh load; explicit developer `start` / `consult` / `consultMany`; maximum **500** entries; reuse of the shared consultation module and existing `CruvitIdentityShadow`; **no** consumers; **no** automatic population; **no** persistence; **no** UI/task/care use; **no** authority change. Do **not** depend on the developer audit as a runtime dependency. Do **not** mutate plants or `plant.meta`. Do **not** persist `plantId`/`canonicalSlug`. Do **not** migrate localStorage. Keep canonical identity advisory and legacy My Garden authoritative. **Growth Outcome Suitability** remains locked after safe canonical read-only adoption (unimplemented). Do not mix Live Search into this phase. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
