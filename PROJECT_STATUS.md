...
| **B** | Canonical plant identities, aliases, and duplicate records | **Frozen (v1)** — **Canonical Identity Foundation v1** is frozen and production-verified (`registryVersion` **1.5.0**; **76** canonical; **1** pending = mulberry; **9** plantIds). Canonical identity remains **advisory**; legacy runtime remains **authoritative**; **no** `plantId`/`canonicalSlug` persistence. **My Garden Phase A** (`CruvitGardenIdentityAudit`) is production verified; **controlled coverage measurement is complete** on isolated synthetic fixtures. Sidecar decision: **GO — architecture planning only** (does **not** authorize Sidecar implementation, persistence, migration, UI/task use, or runtime authority). Next: read-only forensic audit + architecture plan for an advisory in-memory Sidecar. Climate accuracy C–E remain separate. **Later (locked, not started):** Growth Outcome Suitability forensic audit/pilot after canonical read-only adoption is safely completed. |)
...
## High
- Read-only forensic audit and architecture planning for an advisory in-memory My Garden canonical identity Sidecar (**GO — planning only**; coverage measurement complete; implementation / persistence / migration / UI / authority not approved)
...
**Read-only forensic audit and architecture planning for an advisory in-memory My Garden canonical identity Sidecar.** Planning only — authorized by the completed controlled coverage measurement (**GO — architecture planning only**). Do **not** implement the Sidecar; do **not** enable automatic runtime consultation; do **not** mutate plants; do **not** persist `plantId`/`canonicalSlug`; do **not** migrate localStorage; do **not** wire UI/tasks/care; do **not** change runtime authority. Keep `CruvitGardenIdentityAudit` developer-only and disabled by default; legacy My Garden remains authoritative. **Growth Outcome Suitability** remains locked later (unimplemented) until canonical read-only adoption is safely completed. Do not mix Live Search into this planning. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
