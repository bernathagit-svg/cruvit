...
**Other backlog:** confidence-aware scoring refinements; Product/Care Schedule runtime; **Smart Recommendations filter-based UX** (locked — schema foundation only so far; chat and results table unchanged; sun/water logic/UI not started); **Growth Outcome Suitability** (GOS-1 contract complete locally; next = GOS-2A read-only mango-led evidence-source audit and claim mapping; GOS-2B pilot-data entry requires separate approval; no engine/UI/ranking until separately approved).
...
| **17** | Growth Outcome Suitability | **GOS-1 complete locally** (schema/evidence contract). **Next — GOS-2A** read-only mango-led evidence-source audit and claim mapping. GOS-2B pilot-data entry requires separate approval. No engine/catalog/UI/ranking until separately approved. Before broad Smart Recommendations or Add Plant suitability expansion. |
...
- **17 — Growth Outcome Suitability:** GOS-1 schema/evidence contract complete locally. Next = GOS-2A read-only mango-led evidence-source audit and claim mapping (see dedicated plan). Do **not** enter pilot JSON (GOS-2B) or implement engine/catalog/UI/ranking until separately approved.

---

# Growth Outcome Suitability Plan

**Status:** **GOS-1 complete locally** — dedicated profile + evidence schemas, contract validator, and browser harness. Forensic audit / architecture planning for GOS-0 is done. **Next = GOS-2A** read-only mango-led evidence-source audit and claim mapping (no pilot JSON). **GOS-2B** data entry requires explicit approval after GOS-2A review. Do **not** implement an engine, rewrite catalog/`climateTraits`, change UI, or change Smart Recommendations / Add Plant ranking. Sidecar remains inert.

**Placement:** After GOS-1, research and map evidence (GOS-2A) before any pilot dataset (GOS-2B) or shadow evaluator — and **before** broad Smart Recommendations or Add Plant suitability expansion.
...
## High
- GOS-2A — read-only evidence-source audit and claim mapping for the Mango-led pilot (GOS-1 contract complete; no pilot JSON; no schema/engine/UI change; GOS-2B requires separate approval)
...
- Growth Outcome Suitability — **GOS-1 complete locally**; next **GOS-2A** mango-led evidence-source audit and claim mapping (research only). GOS-2B pilot-data entry gated on approval. No engine/catalog/UI/ranking yet. Before broad Smart Recommendations / Add Plant suitability expansion. See dedicated plan.
...
**GOS-2A — read-only evidence-source audit and claim mapping for the Mango-led pilot.** Research and source selection only: map evidence separately for every outcome stage (`survival`, `vegetativeGrowth`, `flowering`, `fruitSet`, `fruitRipeningOrYield`, `longTermReliability`); identify missing, contradictory, regional, and cultivar-specific evidence; propose exact evidence claims **before** JSON data entry. Primary case: **Mango**. Small comparison set from repository plants (e.g. Lychee; Apple or Pear; Olive; Lavender or Bougainvillea). **Do not** create pilot JSON data; **do not** modify GOS schemas; **do not** implement an evaluator; **do not** change catalog, climateTraits, Climate Suitability v1b, UI, Add Plant, Smart Recommendations, Plant Identifier, persistence, or Sidecar. **GOS-2B** pilot-data entry requires explicit approval after review of the GOS-2A evidence map. Sidecar automatic lazy/idle population remains deferred. Keep canonical identity advisory and legacy My Garden authoritative.
