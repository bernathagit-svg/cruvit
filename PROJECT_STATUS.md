...
| **B** | Canonical plant identities, aliases, and duplicate records | **In progress** — Canonical Identity Audit done; Plant Identity Registry Foundation added as **data only**; Stable Plant ID / Dual-Key Migration Audit done (read-only); **Stable Plant ID schema foundation** added (optional opaque `plantId` slot + policy, **no values assigned**); Plant ID Allocation Planning Audit done (read-only); **Model C approved** (registry = authoritative `plantId`/`canonicalSlug`/alias layer, catalog = descriptive layer); **cross-entry validator** added (`scripts/validate-plant-identity.ps1`); **registry expanded to full canonical coverage**; **allocation tooling foundation** + **real nine-ID pilot** (`registryVersion` 1.2.0; 9 opaque `plt_…` IDs); **Shared Plant Identity Resolver Foundation** + **startup-only shadow** + **developer-gated shadow consultation** (legacy remains authoritative; identity advisory); **exact localized-name matching** in the standalone advisory resolver; **first duplicate-conflict identity pilot completed for avocado** (`Persea americana`; `registryVersion` **1.4.0**; canonical **75**; conflicts **2**; **no plantId**); **avocado PLANT_LIBRARY catalog deduplication completed** — exactly one descriptive avocado row remains; `PLANT_INDEX` / `findPlantByName` / legacy + wrapper return the same object for approved lookups; no `plantId`/`canonicalSlug` persisted; strawberry-guava and mulberry remain `pending_conflict`. Next: push and production verification of the avocado catalog deduplication, then a read-only evidence plan for the strawberry-guava scientific spelling conflict (do **not** resolve strawberry-guava yet); dual-key adoption and remaining conflict cleanup remain later |
