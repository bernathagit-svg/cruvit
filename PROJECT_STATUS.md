...
| **B** | Canonical plant identities, aliases, and duplicate records | **In progress** — Canonical Identity Audit done; Plant Identity Registry Foundation added as **data only**; Stable Plant ID / Dual-Key Migration Audit done (read-only); **Stable Plant ID schema foundation** added (optional opaque `plantId` slot + policy, **no values assigned**); Plant ID Allocation Planning Audit done (read-only); **Model C approved** (registry = authoritative `plantId`/`canonicalSlug`/alias layer, catalog = descriptive layer); **cross-entry validator** added (`scripts/validate-plant-identity.ps1`); **registry expanded to full canonical coverage**; **allocation tooling foundation** + **real nine-ID pilot** (`registryVersion` 1.2.0; 9 opaque `plt_…` IDs); **Shared Plant Identity Resolver Foundation** + **startup-only shadow** + **developer-gated shadow consultation** (legacy remains authoritative; identity advisory); **exact localized-name matching** in the standalone advisory resolver; **first duplicate-conflict identity pilot completed for avocado** (`Persea americana`); **avocado PLANT_LIBRARY catalog deduplication completed** (pushed); **first-class scientific-name synonym foundation completed**; **strawberry-guava identity-layer pilot completed** (`acceptedScientificName: Psidium cattleyanum`, synonym `Psidium cattleianum`; `registryVersion` **1.5.0**; canonical **76**; conflicts **1** = mulberry only; **1** real scientific synonym; **no plantId**); **strawberry-guava PLANT_LIBRARY catalog deduplication completed** (exactly one descriptive row; unified lookups; `EXPECTED_RESOLVED_IDENTITY_CATALOG_DUPLICATE` retired for SG). Next: freeze Canonical Identity Foundation v1, then My Garden canonical read-only adoption planning — preserve mulberry pending, `registryVersion` **1.5.0**, no new plantId, resolver advisory, legacy authoritative, no saved-data migration yet |
...
**Freeze Canonical Identity Foundation v1, then begin My Garden canonical read-only adoption planning.** The freeze must preserve: **mulberry** in `pending_conflict`; `registryVersion` **1.5.0**; **no** new plantId allocation; resolver **advisory** status; legacy runtime **authority**; **no** saved-data migration yet. Done so far includes avocado identity + catalog dedupe, the scientific-synonym foundation, the strawberry-guava identity-layer pilot, and **strawberry-guava PLANT_LIBRARY catalog deduplication** (exactly one descriptive row; unified lookups; `registryVersion` **1.5.0**; **76** canonical / **1** pending = mulberry / **9** plantIds / **1** real scientific synonym). Do not allocate a plantId, do not make identity authoritative, and do not migrate saved-user data in the freeze/read-only planning step. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
