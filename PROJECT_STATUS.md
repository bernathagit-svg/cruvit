...
**Other backlog:** confidence-aware scoring refinements; Product/Care Schedule runtime; **Smart Recommendations filter-based UX** (locked — schema foundation only so far; chat and results table unchanged; sun/water logic/UI not started); **Growth Outcome Suitability** (GOS-3C0 comparison feasibility complete locally; next = GOS-3C1 expanded test-only comparison matrix — no UI/ranking/runtime consumer until separately approved).
...
| **17** | Growth Outcome Suitability | **GOS-3C0 complete locally** (v1b/GOS comparison feasibility; READY_FOR_FULL_GOS_3C). **Next — GOS-3C1** expand test-only comparison matrix (five pilot plants + missing-data/applicability). No catalog/UI/ranking/product consumer until separately approved. Before broad Smart Recommendations or Add Plant suitability expansion. |
...
- **17 — Growth Outcome Suitability:** GOS-3C0 comparison feasibility complete locally. Next = GOS-3C1 expand test-only comparison matrix (see dedicated plan). Do **not** add runtime loader/catalog/UI/ranking/product consumer until separately approved.

---

# Growth Outcome Suitability Plan

**Status:** **GOS-3C0 complete locally** — test-only v1b/GOS comparison feasibility harness (`tests/growth-outcome-v1b-comparison.test.html`; READY_FOR_FULL_GOS_3C); GOS-3A/3B evaluator and GOS-2B pilot data remain in force; no application runtime loading. **Next = GOS-3C1** expand the test-only comparison matrix across all five pilot plants and missing-data/applicability cases. Do **not** load pilot data in the application, rewrite catalog/`climateTraits`, change UI, or change Smart Recommendations / Add Plant ranking. Sidecar remains inert and unused by GOS.

**Placement:** After comparison feasibility, expand the fixture matrix (GOS-3C1) before any stopped developer API, product display, or ranking — and **before** broad Smart Recommendations or Add Plant suitability expansion.
...
## High
- GOS-3C1 — expand test-only GOS vs v1b comparison matrix (GOS-3C0 feasibility complete locally; five pilot plants + missing-data/applicability; no app pilot load, UI, ranking, persistence, score bridge, or Sidecar consumer)
...
- Growth Outcome Suitability — **GOS-3C0 complete locally** (comparison feasibility); next **GOS-3C1** expanded test-only comparison matrix. No catalog/UI/ranking/product consumer yet. Before broad Smart Recommendations / Add Plant suitability expansion. See dedicated plan.
...
**GOS-3C1 — expand the test-only comparison matrix across all five pilot plants and missing-data/applicability cases.** Remain test-only and non-judgmental; no production import; no cross-engine scoring bridge; no runtime GOS loading; no UI; no persistence; no Smart Recommendations change; no Sidecar use. Later separately gated: GOS-3D stopped-by-default developer shadow API → GOS-3E production inert verification → GOS-3F explicit developer comparison → GOS-4 first read-only product-display planning. Known Sidecar harness race remains a separate test-infrastructure issue and must be stabilized before any future Sidecar runtime/consumer/persistence phase. Keep canonical identity advisory and legacy My Garden authoritative.
