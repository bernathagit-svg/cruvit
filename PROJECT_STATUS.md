...
| **B** | Canonical plant identities, aliases, and duplicate records | **In progress** — Canonical Identity Audit done; Plant Identity Registry Foundation added as **data only**; Stable Plant ID / Dual-Key Migration Audit done (read-only); **Stable Plant ID schema foundation** added (optional opaque `plantId` slot + policy, **no values assigned**); Plant ID Allocation Planning Audit done (read-only); **Model C approved** (registry = authoritative `plantId`/`canonicalSlug`/alias layer, catalog = descriptive layer); **cross-entry validator** added (`scripts/validate-plant-identity.ps1`); **registry expanded to full canonical coverage**; **allocation tooling foundation** + **real nine-ID pilot** (`registryVersion` 1.2.0; 9 opaque `plt_…` IDs); **Shared Plant Identity Resolver Foundation** + **startup-only shadow** + **developer-gated shadow consultation** (legacy remains authoritative; identity advisory); **exact localized-name matching** in the standalone advisory resolver; **first duplicate-conflict identity pilot completed for avocado** (`Persea americana`; `registryVersion` **1.4.0**; canonical **75**; conflicts **2**; **no plantId**); **avocado PLANT_LIBRARY catalog deduplication completed** (pushed); **first-class scientific-name synonym foundation completed** — optional `scientificSynonyms` on `CanonicalIdentity` (schema + validator + advisory resolver + synthetic tests); **zero** real-registry synonym entries; strawberry-guava and mulberry remain `pending_conflict`. Next: strawberry-guava identity-layer pilot (`acceptedScientificName: Psidium cattleyanum`, synonym `Psidium cattleianum`, structured sources, **no plantId**, **no catalog dedupe**, **no** common-name alias changes; mulberry stays pending; identity remains advisory) — do **not** perform that pilot in the synonym-foundation checkpoint; dual-key adoption and remaining conflict cleanup remain later |
...
**Catalog task B — strawberry-guava identity-layer pilot (not catalog dedupe).** Implement the next quarantined-conflict identity pilot using the new scientific-synonym foundation: promote `strawberry-guava` to a single canonical identity with `acceptedScientificName: Psidium cattleyanum`, scientific synonym `Psidium cattleianum` (structured verified sources), **no plantId**, **no** `localizedNames` / common-name alias changes in that step, **no** catalog/`index.html` descriptive merge, mulberry remains `pending_conflict`, and canonical identity remains advisory. Done so far includes avocado identity + catalog dedupe (pushed), and the **first-class scientific-name synonym foundation** (schema/validator/advisory resolver/synthetic tests; **zero** real-registry synonym entries; `registryVersion` **1.4.0**; **75** canonical / **2** pending / **9** plantIds). Do **not** allocate a plantId, do not make identity authoritative, and do not touch botanical-engine aliases or PLANT_LIBRARY rows in the identity pilot. Dual-key adoption, strawberry-guava catalog deduplication, and mulberry resolution remain later. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
