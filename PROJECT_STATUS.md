...
|------------|--------|-------|
| **Smart Recommendations — Real Lavender and Rosemary Reviewed Climate Profile Catalog Overlay Pilot Implementation** | Done (local; unpushed) | **Exact four-file developer-only inert real-pilot overlay proof; no Product Adapter bridge; no runtime/product authority; no schema/seed/`index.html`/batch/foundation/shared-validator/Batch Draft Generator/synthetic-overlay/synthetic-harness/runtime/`needsReview`/eligibility/matcher/UI/GOS/v1b/media change; no Source Scout; no research.** Files: `data/smart-recommendations-reviewed/overlays/reviewed-climate-profile-catalog-overlay.real-pilot.v1.json`; `modules/smart-recommendations/developer-reviewed-climate-profile-catalog-overlay.js` (minimal `generatedFrom` allowlist); `tests/smart-rec-developer-reviewed-climate-profile-catalog-overlay-real-pilot.test.html`; `PROJECT_STATUS.md`. **Contracts:** overlay `0.1.0-sr-reviewed-climate-profile-catalog-overlay`; integration `0.1.0-sr-catalog-reviewed-climate-profile-integration-contract`; SCP `0.1.0-sr-structured-climate-profile-contract`; catalogSchemaVersion **1**; identityRegistryVersion **1.5.0**. **Allowlist:** `SR_REVIEWED_CLIMATE_PROFILE_GENERATED_FROM_ALLOWLIST = Object.freeze(['synthetic_fixture','real_reviewed_batch_pilot'])`; normalized output preserves accepted `generatedFrom` (no rewrite to synthetic). **Real overlay:** `generatedFrom: real_reviewed_batch_pilot`; records ordered `lavender`,`rosemary`; both `inert_imported`; all authority flags false (`productAuthority`/`eligibilityAuthority`/`scalarAuthority`/`runtimeRecommendationAuthority`/`GOSOutcomeAuthority`). **Lavender:** `fields.sun`; identity `lavender\|Lavandula angustifolia\|1.5.0\|false\|false`; profileId `scp-lavender-sun-preference-v1`; value `full_sun` preference; context outdoor/ground/mature/general; region omitted; evidenceRefCount **2**; batchId `lavender-sun-preference-v1`; batch FP `0.1.0-sr-reviewed-data-batch-validator\|0.1.0-sr-reviewed-data-container\|1\|2\|1\|1\|0\|0\|0\|\|passed\|lavender-sun-preference-v1`; limitation `partial_shade_listed_as_possible_tolerance_or_context_only_not_a_second_reviewed_claim`; compact `profileFingerprint` via `buildReviewedClimateProfileCompactFingerprint` (≠ raw SCP FP); FR compact reference bound to committed FR. **Rosemary:** `fields.water`; identity `rosemary\|Salvia rosmarinus\|1.5.0\|false\|false`; profileId `scp-rosemary-water-preference-v1`; value `low` preference; same context; region omitted; evidenceRefCount **2**; batchId `rosemary-water-preference-v1`; batch FP `…\|passed\|rosemary-water-preference-v1`; limitations sorted `container_watering_is_outside_the_ground_context`, `establishment_year_watering_is_outside_the_mature_context`, `ncsu_dry_to_medium_band_is_not_a_second_reviewed_medium_claim`; compact FP via builder; FR bound. **Informational legacy agreement only:** lavender sun `full_sun`; rosemary water `low` — no climateTraits/seed/v1b mutation; no effective/resolved/recommended values. **reportFingerprint:** `0.1.0-sr-reviewed-climate-profile-catalog-overlay\|\|\|lavender,rosemary`. **Harness:** separate explicit-run; globals `__SR_REVIEWED_CLIMATE_PROFILE_CATALOG_OVERLAY_REAL_PILOT_BOOT` / `_LAST`; Reset DOM (`#banner`/`#bannerStatus`/`#bootMarker`/`ensureHarnessBannerStructure`/`setBannerState`/`restoreInertUi`); inert summary `Not run yet.`; request budget after Run = module + real overlay + lav SCP + lav FR + ros SCP + ros FR (**6** same-origin; Reset **0**). **Local five identical Reset→Run→Reset cycles:** **118/118**; verdict `REAL_REVIEWED_CLIMATE_PROFILE_CATALOG_OVERLAY_PASS`; records **2**; status both `inert_imported`; hard errors **0**; warnings/findings **0**; counters overlayRequests **1** / sourceArtifactRequests **4** / external **0** / storage **0** / console **0** / unhandled **0** / runtimeIntegration **0** / productAuthority **0** / v1b **0** / GOS **0** / smartRecommendationsMutation **0**; summaryFingerprint identical across 5 cycles (`0.1.0-sr-reviewed-climate-profile-catalog-overlay\|pass=118\|fail=0\|records=2\|generatedFrom=real_reviewed_batch_pilot\|overlayRequests=1\|sourceArtifactRequests=4\|external=0\|storage=0\|0.1.0-sr-reviewed-climate-profile-catalog-overlay\|\|\|lavender,rosemary\|` + lav/ros compact fingerprints). **Non-regression unchanged:** synthetic overlay **98/98** (`generatedFrom===synthetic_fixture`); Product Adapter **112/112**; Batch Draft Generator **92/92**; Lavender **64/64**; Rosemary **64/64**; merge **89/89**; recon **158**/156 PASS/2 quarantined/0 unexpected; resolver **108/108**; legacy **36/36/36**; **23** conflicts blocked; leakage **0**; identity registry **1.5.0**; foundation defaults **0/0/0**; trusted-location/missing-meta unchanged; v1b product authority; GOS paused. **Product isolation:** `index.html` unchanged (no real/synthetic overlay, overlay-module, real-pilot harness, or Product Adapter import); normal app resource list has **zero** overlay/adapter hits; no runtime `reviewedClimateProfile` read; no recommendation/score/ranking/filter/matcher/eligibility/UI/v1b/GOS change; no Adapter bridge; no Source Scout. **Next checkpoint:** push and production verification. **Deferred:** Real Overlay → Product Adapter Bridge; Source Scout. Verdict: **`REAL_REVIEWED_CLIMATE_PROFILE_CATALOG_OVERLAY_IMPLEMENTED`**. |
...
- Real-pilot overlay artifact + harness are implemented locally (this commit) — see Completed Checkpoints.
...
- Source Scout remains deferred.
- **Next:** push and production verification. Real Overlay → Product Adapter Bridge remains deferred.
...
**Done (local; this commit):** Real Lavender and Rosemary Reviewed Climate Profile Catalog Overlay Pilot Implementation — four-file developer-only inert real-pilot overlay (`lavender` sun + `rosemary` water); `generatedFrom` allowlist `{synthetic_fixture, real_reviewed_batch_pilot}`; separate explicit-run harness; five identical local Reset→Run→Reset cycles **118/118** / `REAL_REVIEWED_CLIMATE_PROFILE_CATALOG_OVERLAY_PASS`; synthetic **98/98** and Product Adapter **112/112** unchanged; no runtime/Product Adapter/v1b/Smart Recommendations/GOS integration. Verdict: `REAL_REVIEWED_CLIMATE_PROFILE_CATALOG_OVERLAY_IMPLEMENTED`.

**Next (exactly one):** **Push and production verification** of this real-pilot overlay implementation (separately gated). Do **not** begin Real Overlay → Product Adapter Bridge, Source Scout, media track, third reviewed-data batch, adapter→v1b/Smart Recommendations/GOS integration, or product eligibility in this commit. UI remains blocked. GOS remains paused. Blueberry remains excluded. Lavender and Rosemary batches remain developer-only and inert. Real-pilot overlay remains developer-only and inert. All **23** conflicts remain blocked. v1b + `climateTraits` remain product climate authority. Product Adapter remains developer-only and non-authoritative. Synthetic overlay remains production-verified and untouched.
