...
**Catalog task B — canonical plant identities: expand the registry to cover all canonical catalog identities (data only, no IDs assigned).** Done so far: Canonical Identity Audit; **data-only Plant Identity Registry Foundation** (`data/plant-identity.registry.json` + `data/plant-identity.schema.json`); **Stable Plant ID / Dual-Key Migration Audit** (read-only); **Stable Plant ID schema foundation** — optional opaque `plantId` slot (`^plt_[a-z0-9]{16}$`) added to `CanonicalIdentity` with policy, **no values assigned**; **Plant ID Allocation Planning Audit** (read-only) with **Model C approved** (registry = authoritative `plantId`/`canonicalSlug`/alias layer, catalog = descriptive layer); **cross-entry validator** `scripts/validate-plant-identity.ps1` (read-only; 28 checks PASS; enforces global/registry-wide uniqueness, quarantine, and referential consistency). The registry is **not wired into runtime** and same-slug duplicates remain **documented, not resolved**. The next identity step is to **expand the registry to cover all canonical catalog identities as data (canonicalSlug + acceptedScientificName + needsReview, still NO `plantId` values assigned)**; a separately-approved pilot ID allocation, dual-key adoption, and duplicate resolution are later tasks (`avocado`/`strawberry-guava`/`mulberry` stay ineligible until explicit duplicate-resolution decisions; the 16 genus-level/Various identities need editorial review). Keep `profileSlug`/`meta.slug` for saved My Garden plants. Do not rename, merge, or delete records, do not modify saved user plants, and do not populate SR taxonomy arrays or implement filter UI in these identity steps. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
