...
|------------|--------|-------|
| **Smart Recommendations — Inert reviewedClimateProfile Overlay Harness Reset-State Fix** | Done (local) | **Harness-only fix; no schema/overlay/module/seed/index/runtime/Product Adapter/authority change; production verification not completed in this commit and must be rerun after a separate push.** Root cause of failed prod-verify on deployed `b281818`: PASS rendering replaced `#banner` via `innerHTML`, deleting `#bootMarker`; subsequent `resetHarness()` threw before restoring visible inert UI/`Not run yet.` (globals cleared, stale PASS summary remained). Fix: stable banner structure (`#bannerStatus` + preserved `#bootMarker`), `ensureHarnessBannerStructure()` / `setBannerState()` / `restoreInertUi()`, idempotent Reset, PASS/FAIL no longer destroy children; explicit Reset-after-PASS regression asserts. Local harness now **98/98**; verdict still `INERT_REVIEWED_CLIMATE_PROFILE_CATALOG_OVERLAY_PASS`; five complete Reset→Run→Reset cycles identical; overlay **1**/Run; Reset fetch **0**; external/storage/console/unhandled **0**; warnings unchanged (`needs_review_active`, `climate_traits_value_conflict`). Non-regression unchanged: merge **89/89**; automation **92/92**; Lavender/Rosemary **64/64**; recon **158/156/2/0**; resolver **108/108**. Protected schema/overlay/module/seed/index byte-identical. **Next:** push this fix, then rerun isolated production verification. |
| **Smart Recommendations — Inert reviewedClimateProfile Schema Implementation** | Done (pushed; prod-verify blocked by harness Reset) | **Five-file developer-only inert catalog proof; pushed at `b281818`.** Integration `0.1.0-sr-catalog-reviewed-climate-profile-integration-contract`; overlay `0.1.0-sr-reviewed-climate-profile-catalog-overlay`; catalog `schemaVersion` **1**. Optional sibling `reviewedClimateProfile`; synthetic records `synthetic-rcp-sun-agree` / `synthetic-rcp-water-conflict-blocked`. Initial local harness baseline **78/78**. Isolated production verification on `b281818` matched deploy content and Run **78/78** ×5 with full non-regression/isolation, but **failed** the Reset→visible-inert gate (harness DOM only). Schema/overlay/module/product baselines were not at fault. Harness Reset fix is the following checkpoint. |
...
**Checkpoint type:** harness-only local fix. **Files:** proof harness + `PROJECT_STATUS.md` only.
...
**Closure verdict identity:** `INERT_REVIEWED_CLIMATE_PROFILE_HARNESS_RESET_FIXED_LOCALLY`

## Root cause

PASS rendering used `$('banner').innerHTML = 'PASS…'`, which deleted `#bootMarker`. `resetHarness()` then accessed `$('bootMarker').textContent` and threw before restoring the visible inert banner/`Not run yet.` summary, leaving a stale PASS display while clearing globals.

## Fix

- Stable banner children: `#bannerStatus` + `#bootMarker` (preserved across inert/running/PASS/fail).
- `ensureHarnessBannerStructure()`, `setBannerState()`, `restoreInertUi()`.
- Idempotent null-safe Reset restores boot.inert, LAST null, marker, inert status, summary `Not run yet.`, clean styling, zero fetch.
- Explicit Reset-after-PASS regression coverage inside the harness.

## Local results

- Assertions: **98/98** (was 78; +Reset-after-PASS / next-run coverage)
- Verdict: `INERT_REVIEWED_CLIMATE_PROFILE_CATALOG_OVERLAY_PASS`
- Five complete Reset→Run→Reset cycles: identical
- Overlay request: **1** per Run; Reset fetch: **0**
- external/storage/console/unhandled: **0**
