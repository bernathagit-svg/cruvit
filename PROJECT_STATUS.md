...
| **B** | Canonical plant identities, aliases, and duplicate records | **In progress** — Canonical Identity Audit done (read-only); Plant Identity Registry Foundation added as **data only** (`data/plant-identity.registry.json` + `data/plant-identity.schema.json`), not wired to runtime; next is stable `plantId` + dual-key migration planning, then alias wiring and duplicate cleanup |
...
**Catalog task B — canonical plant identities: stable `plantId` + dual-key migration planning.** The Canonical Identity Audit and the **data-only Plant Identity Registry Foundation** (`data/plant-identity.registry.json` + `data/plant-identity.schema.json`) are done; the registry is **not wired into runtime**, and same-slug duplicates are **documented, not resolved**. The next identity step is **planning a stable `plantId` and dual-key migration** (keep `profileSlug`/`meta.slug` for saved My Garden plants while introducing `plantId`). Alias wiring and duplicate cleanup come **only after** that planning. Do not rename, merge, or delete records, do not modify saved user plants, and do not populate SR taxonomy arrays or implement filter UI in these identity steps. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
