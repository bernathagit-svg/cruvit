...
| **B** | Canonical plant identities, aliases, and duplicate records | **Frozen (v1)** — **Canonical Identity Foundation v1** is frozen and production-verified (`registryVersion` **1.5.0**; **76** canonical; **1** pending = mulberry; **9** plantIds). Canonical identity remains **advisory**; legacy runtime remains **authoritative**; **no** `plantId`/`canonicalSlug` persistence. **My Garden Phase A** production verified; coverage measurement complete; Sidecar architecture planning complete; **Phase C1 production verified** at `d239249`; **Phase C2 complete locally** — inert developer-only in-memory Sidecar (`CruvitGardenIdentitySidecar`) stopped by default; explicit consult only; **no runtime consumer / auto-population / persistence / UI / authority**. Next: production-safe rollout verification of stopped Sidecar → go/no-go for later non-authoritative lazy/idle population. Climate accuracy C–E remain separate. **Later (locked, not started):** Growth Outcome Suitability forensic audit/pilot after canonical read-only adoption is safely completed. |)
...
**Other backlog:** confidence-aware scoring refinements; Product/Care Schedule runtime; **Smart Recommendations filter-based UX** (locked — schema foundation only so far; chat and results table unchanged; sun/water logic/UI not started); **Growth Outcome Suitability** (locked later — forensic audit + evidence-backed pilot only after canonical read-only adoption is safely completed; do not design/implement now).
...
|------------|--------|-------|
| **My Garden Canonical Read-Only Adoption — Phase C2 complete locally** | Done (local) | **Inert developer-only in-memory Sidecar; no runtime consumer, automatic population, plant mutation, persistence, migration, UI/task/care use, or authority change.** Implemented `modules/identity/my-garden-identity-sidecar.js` + harness `tests/my-garden-identity-sidecar.test.html`; `index.html` adds exactly one ES-module script after the audit/shared-loading path. Frozen global `window.CruvitGardenIdentitySidecar` API: `start`, `stop`, `isStarted`, `consult`, `consultMany`, `get`, `delete`, `clear`, `status`, `snapshot`. **Fresh load:** stopped, size 0, empty snapshot; no consultation, garden iteration, storage/network access, console output, timers, observers, or listeners; no URL/query/localStorage/sessionStorage/cookie/BroadcastChannel/service-worker/cross-tab activation. **Lifecycle:** `start()` idempotent and does not populate; `stop()` idempotent, clears entries; reload restores stopped/empty. **Key/validity:** Map keyed by normalized `GardenPlant.id` (never confused with registry `plantId`); entry validity requires matching id + `registryVersion` + fingerprint from shared `buildGardenPlantIdentityFingerprintInput` (no second fingerprint algorithm); resolution stored from `resolveGardenPlantIdentityReadOnly` + existing `CruvitIdentityShadow` without rewriting. **Cache ops:** `stored` / `cache_hit` / `refreshed`; `get(plant)` rejects/removes stale fingerprint or registry mismatch; `registry_not_ready` / `registry_error` not permanently cached; missing id never minted. **Bounds/collisions:** max **500** entries; no silent eviction; 501st new id → `capacity_reached`; `consultMany` processes ≤500 with truncation count; same id + different fingerprints → `garden_plant_id_collision` for all conflicting batch members and no retained entry (no majority vote / array-order choice). **Quarantine preserved:** Mulberry `pending_conflict` with null ids; conflicting signals → `ambiguous` / `conflicting_signals` with null ids; provisional/unresolved advisory; no fabricated plantId; no legacy substring fallback. Entries/operation results/batch reports/status/snapshots deeply frozen. **Verified:** Sidecar **126/126 PASS**; audit **126/126**; resolver **108/108**; reconciliation **158 / 156 PASS / 2 EXPECTED_QUARANTINED_CONFLICT / 0 unexpected**; validator **81 PASS** exit 0; network deltas fetch/XHR/beacon/WebSocket/registry all **0**; `registryVersion` **1.5.0**; **76** canonical; **1** pending; **9** plantIds. **Performance (this machine, medians):** single ~0 ms; consultMany 20 ~0.25 ms; 100 ~1.0 ms; 500 ~6.6 ms (~0.013 ms/plant); cache hit ~0 ms; clear ~0 ms; stopped idle size 0; 500-fill below 50 ms gate. **Explicit exclusions:** no automatic start; no garden-load/render/normalize/task/save/Add Plant/Plant Identifier/Smart Rec/Garden Design/Plant Doctor/care integration; no idle/`requestIdleCallback` prefill; no persistence of Sidecar state / plantId / canonicalSlug / fingerprints / entries; no migration; no UI badges/consumers; no runtime authority promotion. **Rollback** = remove Sidecar script tag + delete Sidecar module/harness + revert docs. **Next:** production-safe rollout and verification of the stopped-by-default inert Sidecar, then explicit go/no-go for a later non-authoritative lazy/idle population phase (production verification of C2 does **not** approve automatic population or any consumer). **Growth Outcome Suitability** remains locked after safe canonical read-only adoption. |
...
| **17** | Growth Outcome Suitability | **Future / locked** — dedicated forensic audit + evidence-backed pilot **only after** My Garden canonical read-only adoption is safely completed; before broad Smart Recommendations or Add Plant suitability expansion. Not designed or implemented yet. |
...
- **17 — Growth Outcome Suitability:** locked later requirement — see dedicated plan below. Do **not** design or implement until My Garden canonical read-only adoption is safely completed.

---

# Growth Outcome Suitability Plan

**Status:** Locked later roadmap requirement — **documented only**. Do **not** design or implement during Sidecar architecture planning. Do **not** begin this layer until My Garden canonical read-only identity adoption is safely completed.

**Placement:** After canonical read-only adoption is safely completed, perform a dedicated Growth Outcome Suitability forensic audit and evidence-backed pilot **before** broad Smart Recommendations or Add Plant suitability expansion.

**Future layer must distinguish (when eventually designed):**
- survival
- vegetative growth
- flowering
- fruiting/yield
...
- confidence, sources, region applicability, and unknown fallback
...
## High
- Production-safe rollout and verification of the stopped-by-default inert Sidecar (Phase C2 complete locally), then go/no-go for later non-authoritative lazy/idle population (does **not** approve automatic population or consumers)
...
- Growth Outcome Suitability — **locked later**; forensic audit + evidence-backed pilot only after My Garden canonical read-only adoption is safely completed, and before broad Smart Recommendations / Add Plant suitability expansion. See dedicated plan. Not designed or implemented yet.
...
**Production-safe rollout and verification of the stopped-by-default inert Sidecar, followed by an explicit go/no-go decision for a later non-authoritative lazy or idle population phase.** Verify in production that `CruvitGardenIdentitySidecar` remains stopped and empty on fresh load, that explicit developer consultation works without mutating plants/persistence/UI/tasks, and that no automatic population or consumer is present. Production verification of C2 does **not** approve automatic population or any consumer. Do **not** wire garden-load/render/task/save paths; do **not** add idle/`requestIdleCallback` prefill without a separate go decision; do **not** persist Sidecar state, `plantId`, or `canonicalSlug`; do **not** migrate localStorage; do **not** change runtime authority. Keep canonical identity advisory and legacy My Garden authoritative. **Growth Outcome Suitability** remains locked after safe canonical read-only adoption (unimplemented). Do not mix Live Search into this phase. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
