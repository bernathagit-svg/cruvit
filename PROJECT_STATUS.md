...
| **B** | Canonical plant identities, aliases, and duplicate records | **Frozen (v1)** — **Canonical Identity Foundation v1** is frozen and production-verified at commit `3abdde5` (`registryVersion` **1.5.0**; **76** canonical; **1** pending conflict = mulberry; **9** plantIds; **2** localized-name identities / **2** Hebrew locale blocks; **1** indexed scientific synonym). Canonical identity remains **advisory**; legacy runtime remains **authoritative**; **no** `plantId`/`canonicalSlug` persistence. Next for this track: **read-only audit and implementation planning** for My Garden canonical read-only identity adoption (planning only — no persistence, migration, UI, or authority change). Climate accuracy C–E remain separate. |)
...
**Read-only audit and implementation planning for My Garden canonical read-only identity adoption.** The next phase is **read-only planning first** — do **not** implement adoption, persist `plantId`, add `canonicalSlug` to saved objects, migrate localStorage, change UI, or make the resolver authoritative. Planning must determine: every current My Garden plant-object shape; how `profileSlug`, `scientific`, `name`, `meta`, and `source` are stored; every creation/load/update path; where `resolvePlantProfileRaw` currently runs; how `CruvitIdentityShadow` or the shared resolver can be consulted **in memory** without modifying saved objects; safe resolution statuses (`resolved_id`, `resolved_canonical`, `pending_conflict`, `ambiguous`, `provisional`, `unresolved`); how unknown or conflicted plants remain unchanged; how to measure resolution coverage; performance impact; rollback; and tests required before any implementation. Canonical Identity Foundation v1 remains frozen at `registryVersion` **1.5.0** (advisory identity; legacy authoritative; mulberry pending). Do not mix the separate Add Plant Live Search UX issue into this planning task. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
