...
|------------|--------|-------|
| **My Garden Canonical Read-Only Identity Adoption — Phase A complete locally** | Done (local; not pushed) | **Developer-only in-memory audit; not a runtime consumer of canonical identity.** Added frozen global `window.CruvitGardenIdentityAudit` (`modules/identity/my-garden-identity-audit.js`) with `enable` / `disable` / `isEnabled` / `run` / `runStored` / `latest` / `snapshot` / `clear` / `status`. **Disabled on every fresh load**; no query-string / localStorage / sessionStorage / cookie activation; **no automatic execution** on app load, garden load, render, or task sync — explicit `enable()` + `run()` / `runStored()` only. Consults existing `CruvitIdentityShadow` only (no second resolver, no extra registry GET). Independent signal consultation across plantId, canonicalSlug, profileSlug, meta.slug, slug, scientific, meta.scientific, he, meta.he, name, meta.name; pending-conflict protection; conflicting canonical signals → `ambiguous` / `auditReason: conflicting_signals` (no majority vote); max **500** plants/run; max **10** retained reports; deeply frozen results; `runStored` verifies raw `cruvit_mg_v23_real_plant_profiles` byte parity and never creates the key or demo garden. `index.html` adds exactly one module script after the shadow adapter. Harness: `tests/my-garden-identity-audit.test.html`. **No** UI, task, persistence, saved-object, migration, or authority change. **Exclusions:** no sidecar; no automatic runtime consumer; no `plantId`/`canonicalSlug` persistence; no saved-data migration; no task/care canonical joins; no Shared Plant Picker; no Live Search fix; no authority change. Verified: audit harness **76/76 PASS**; resolver **108/108 PASS**; reconciliation **158 total — 156 PASS, 2 EXPECTED_QUARANTINED_CONFLICT, 0 unexpected**; validator **81 PASS exit 0**; `git diff --check` clean; `registryVersion` **1.5.0**; **76** canonical; **1** pending conflict; **9** plantIds. **Rollback** = remove audit script tag + delete audit module/harness + revert this documentation. **Next recommended task:** production-safe rollout and verification of the disabled developer-only Phase A audit, followed by controlled coverage measurement on isolated test/demo gardens. **Sidecar planning is not approved yet.** |
...
**Production-safe rollout and verification of the disabled developer-only Phase A audit, followed by controlled coverage measurement on isolated test/demo gardens.** Keep `CruvitGardenIdentityAudit` disabled by default; verify fresh production/local loads do not auto-run; then measure coverage only on isolated test/demo gardens via explicit `enable()` + `run()` / `runStored()`. Do **not** persist `plantId` or `canonicalSlug`; do **not** migrate saved data; do **not** change UI, tasks, or authority. **Sidecar planning is not approved yet.** Do not mix the separate Add Plant Live Search UX issue into this work. Canonical Identity Foundation v1 remains frozen at `registryVersion` **1.5.0**. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
