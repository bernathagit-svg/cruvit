...
**Other backlog:** confidence-aware scoring refinements; Product/Care Schedule runtime; **Smart Recommendations filter-based UX** (locked — schema foundation only so far; chat and results table unchanged; sun/water logic/UI not started); **Growth Outcome Suitability** (GOS-3A/3B isolated pure evaluator complete locally; next = GOS-3C isolated v1b comparison harness — test-only; no UI/ranking/runtime consumer until separately approved).
...
| **17** | Growth Outcome Suitability | **GOS-3A/3B complete locally** (isolated pure evaluator + harness; no runtime loader). **Next — GOS-3C** isolated comparison harness against Climate Suitability v1b (test-only). No catalog/UI/ranking/product consumer until separately approved. Before broad Smart Recommendations or Add Plant suitability expansion. |
...
- **17 — Growth Outcome Suitability:** GOS-3A/3B isolated pure evaluator complete locally. Next = GOS-3C isolated comparison harness against Climate Suitability v1b (test-only; see dedicated plan). Do **not** add runtime loader/catalog/UI/ranking/product consumer until separately approved.

---

# Growth Outcome Suitability Plan

**Status:** **GOS-3A/3B complete locally** — isolated pure evaluator (`modules/suitability/growth-outcome-evaluator.js`, `0.1.0-gos3a`) + standalone harness; GOS-2B pilot data and GOS-1 schemas remain in force; no application runtime loading. **Next = GOS-3C** isolated comparison harness against Climate Suitability v1b (**test-only**). Do **not** load pilot data in the application, rewrite catalog/`climateTraits`, change UI, or change Smart Recommendations / Add Plant ranking. Sidecar remains inert and unused by GOS.

**Placement:** After the isolated evaluator, compare GOS vs v1b on explicit fixtures (GOS-3C) before any stopped developer API, product display, or ranking — and **before** broad Smart Recommendations or Add Plant suitability expansion.
...
## High
- GOS-3C — isolated comparison harness against Climate Suitability v1b (GOS-3A/3B evaluator complete locally; test-only; no app pilot load, UI, ranking, persistence, or Sidecar consumer)
...
- Growth Outcome Suitability — **GOS-3A/3B complete locally** (isolated pure evaluator); next **GOS-3C** isolated v1b comparison harness (test-only). No catalog/UI/ranking/product consumer yet. Before broad Smart Recommendations / Add Plant suitability expansion. See dedicated plan.
...
**GOS-3C — isolated comparison harness against Climate Suitability v1b.** Test-only: compare GOS evaluator outputs with Climate Suitability v1b on explicit fixtures; document where v1b and GOS answer different questions; verify current v1b behavior remains unchanged; verify Smart Recommendations remains unchanged; do **not** choose a winner automatically; do **not** load pilot data in the application; do **not** add UI, runtime consumer, persistence, or Sidecar use. Later separately gated: GOS-3D stopped-by-default developer shadow API → GOS-3E production inert verification → GOS-3F explicit developer comparison → GOS-4 first read-only product-display planning. Known Sidecar harness race remains a separate test-infrastructure issue and must be stabilized before any future Sidecar runtime/consumer/persistence phase. Keep canonical identity advisory and legacy My Garden authoritative.
