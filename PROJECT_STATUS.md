...
**Catalog task B — read-only planning for strawberry-guava catalog deduplication.** Plan only (no `index.html` / catalog edits in the planning step): field-by-field comparison of the two remaining strawberry-guava `PLANT_LIBRARY` rows; Record B survivor analysis; accepted scientific spelling (`Psidium cattleyanum`); cherry-guava alias decision; regional invasive-warning policy; warning/product/care merge proposal; botanical-engine consistency check. Done so far includes avocado identity + catalog dedupe (pushed), the scientific-synonym foundation, and the **strawberry-guava identity-layer pilot** (`registryVersion` **1.5.0**; **76** canonical / **1** pending = mulberry / **9** plantIds / **1** real scientific synonym; temporary catalog duplication classified as `EXPECTED_RESOLVED_IDENTITY_CATALOG_DUPLICATE`). Do **not** perform catalog deduplication until that plan is approved; do not allocate a plantId; do not make identity authoritative; mulberry remains pending. Then climate accuracy C–E; sun/water filter logic/UI only when separately approved.
