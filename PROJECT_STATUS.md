...
**Other backlog:** confidence-aware scoring refinements; Product/Care Schedule runtime; **Smart Recommendations filter-based UX** (locked — schema foundation only so far; chat and results table unchanged; sun/water logic/UI not started); **Growth Outcome Suitability** (GOS-2B Mango-led pilot dataset complete locally; next = GOS-3 pure shadow evaluator forensic planning and algorithm contract — read-only; no engine/UI/ranking until separately approved).
...
| **17** | Growth Outcome Suitability | **GOS-2B complete locally** (Mango-led pilot dataset; isolated validation; no runtime loader). **Next — GOS-3** pure shadow evaluator forensic planning and algorithm contract (read-only). No engine/catalog/UI/ranking until separately approved. Before broad Smart Recommendations or Add Plant suitability expansion. |
...
- **17 — Growth Outcome Suitability:** GOS-2B Mango-led pilot dataset complete locally. Next = GOS-3 pure shadow evaluator forensic planning and algorithm contract (read-only; see dedicated plan). Do **not** implement evaluator/runtime loader/catalog/UI/ranking until separately approved.

---

# Growth Outcome Suitability Plan

**Status:** **GOS-2B complete locally** — Mango-led pilot evidence/profiles with isolated validator + browser harness; GOS-1 schema/evidence contract remains in force; GOS-1 validator coexistence assert updated without weakening its contract. **Next = GOS-3** pure shadow evaluator forensic planning and algorithm contract (**read-only** investigation/planning only). Do **not** implement an evaluator, load pilot data at runtime, rewrite catalog/`climateTraits`, change UI, or change Smart Recommendations / Add Plant ranking. Sidecar remains inert and unused by GOS.

**Placement:** After the GOS-2B pilot dataset, plan the pure shadow evaluator algorithm (GOS-3) before any runtime evaluator, product display, or ranking — and **before** broad Smart Recommendations or Add Plant suitability expansion.
...
## High
- GOS-3 — pure shadow evaluator forensic planning and algorithm contract (GOS-2B pilot complete locally; read-only; no evaluator implementation, runtime load, UI, ranking, or Sidecar consumer yet)
...
- Growth Outcome Suitability — **GOS-2B complete locally** (Mango-led pilot dataset); next **GOS-3** pure shadow evaluator forensic planning and algorithm contract (read-only). No engine/catalog/UI/ranking yet. Before broad Smart Recommendations / Add Plant suitability expansion. See dedicated plan.
...
**GOS-3 — pure shadow evaluator forensic planning and algorithm contract.** Read-only investigation and planning only: define how a future pure shadow evaluator would consult GOS profiles/evidence for the six outcomes without mutating product behavior; specify algorithm/contract boundaries, unknown/conflict handling, applicability gates, and non-goals. **Do not** implement the evaluator; **do not** load pilot data at runtime; **do not** modify the product, Climate Suitability v1b, Smart Recommendations ranking, Add Plant, Plant Identifier, Garden Design, tasks, care, persistence, identity, or Sidecar; **do not** display GOS results; **do not** persist computed outcomes; **do not** auto-consume the Sidecar. Known Sidecar harness race remains a separate test-infrastructure issue and must be stabilized before any future Sidecar runtime/consumer/persistence phase. Keep canonical identity advisory and legacy My Garden authoritative.
