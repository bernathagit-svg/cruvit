...
function familySection(id, title, slots, note) {
  const slotHtml = slots
    .map((slot) => {
      const src = slot.src ? esc(slot.src) : '';
      const inner = src
        ? `<img class="cutout" alt="${esc(slot.label)}" src="${src}"/>`
        : `<div class="ghost blocked">${esc(slot.empty || 'ASSET NOT GENERATED')}</div>`;
      return `<div class="slot" data-job-id="${esc(slot.jobId)}">
...
export function buildVisualStateCalibrationBatch2ReviewHtml(options = {}) {
  ...
  for (const job of options.generatedJobs || []) {
    if (job && job.jobId && job.file && job.generated) {
      generated.set(job.jobId, liveCutoutSrc(job.file));
    }
  }
  const generatedCount = generated.size;
  const slot = (jobId, label) => ({
    jobId,
    label,
    src: generated.get(jobId) || '',
    empty: generated.get(jobId) ? '' : 'ASSET NOT GENERATED'
  });
  ...
  const title = generatedCount
    ? `Visual State Calibration Batch 2 — ${generatedCount}/11 candidates, owner review only`
    : 'Visual State Calibration Batch 2 — 11 jobs, five pair-complete families, 0 generated';
  const banner = generatedCount
    ? 'Candidates only. Not approved. Signed Garden photo is injected by the host. Blend V1 CSS is runtime-only. Physical size remains runtime authority; families are not forced to one scale.'
    : 'No assets yet. Signed Garden photo is injected by the host when available. Do not bake scene integration into PNGs. Blend/ground-contact is runtime CSS only. Physical size remains runtime authority; families are not forced to one scale.';
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
  ...
  <style>
    ...
    .scene img.cutout { position: absolute; left: 50%; bottom: 8%; transform: translateX(-50%); max-height: 82%; max-width: 72%; object-fit: contain; pointer-events: none; }
...
<body>
  ...
  <p class="note">Spend gate DENIED after this run. Production registry unchanged. Auto-approval disabled. QA: TECHNICAL_QA, BOTANICAL_IDENTITY_QA, STATE_QA, FAMILY_CONSISTENCY_QA, IN_GARDEN_QA, OWNER_VISUAL_QA. STATE / FAMILY / OWNER visual judgments stay UNKNOWN until owner review.</p>
${familySection('A', 'Mango — 3-way', [
    slot('mango__mature__tree__vegetative__v1', '1. TREE MATURE VEGETATIVE (new family anchor)'),
    slot('mango__young__tree__vegetative__v1', '2. TREE YOUNG VEGETATIVE'),
    slot('mango__mature__tree__fruiting__v1', '3. TREE MATURE FRUITING')
...
${familySection('B', 'Banana — 2-way', [
    slot('banana__mature__default__vegetative__v1', '4. herbaceous-clump MATURE VEGETATIVE (new family anchor)'),
    slot('banana__young__default__vegetative__v1', '5. herbaceous-clump YOUNG VEGETATIVE')
...
${familySection('C', 'Apple — 2-way', [
    slot('apple__mature__tree__vegetative__v1', '6. TREE MATURE VEGETATIVE'),
    slot('apple__mature__tree__dormant__v1', '7. TREE MATURE DORMANT')
  ], 'Proves VEGETATIVE vs DORMANT. Young Apple is not in this batch.')}
${familySection('D', 'Pomegranate — 2-way', [
    slot('pomegranate__mature__tree__vegetative__v1', '8. TREE MATURE VEGETATIVE'),
    slot('pomegranate__mature__shrub__vegetative__v1', '9. SHRUB MATURE VEGETATIVE')
  ], 'Proves TREE vs SHRUB architectureMode. One canonical identity.')}
${familySection('E', 'Lavender — 2-way', [
    slot('lavender__mature__shrub__vegetative__v1', '10. SHRUB MATURE VEGETATIVE (new family anchor)'),
    slot('lavender__mature__shrub__flowering__v1', '11. SHRUB MATURE FLOWERING')
  ...
  <script>
    ...
    function applySignedUrl(url) {
      ...
      if (banner) {
        banner.className = 'ok';
        banner.textContent = ${JSON.stringify(
          generatedCount
            ? 'Real Garden photo loaded. Candidates only — not approved. Blend V1 CSS is runtime-only. No bake-in.'
            : 'Real Garden photo loaded. Assets are not generated. Blend V1 CSS will apply when cutouts exist. No bake-in.'
        )};
      }
      return true;
    }
...
export function writeVisualStateCalibrationBatch2Review(root, options = {}) {
  ...
  fs.writeFileSync(htmlPath, `${buildVisualStateCalibrationBatch2ReviewHtml(options)}`);
  const generatedCount = (options.generatedJobs || []).filter((job) => job && job.generated && job.file).length;
  return {
    htmlPath,
