...
export function familyLockLines(job = {}) {
  ...
  return [
    ...
    'FRAMING SAFE ZONE: the complete visible specimen must sit comfortably inside the canvas and must not touch any image boundary. Center the specimen horizontally. Keep the visible plant width to no more than about 90% of the canvas and the visible plant height to no more than about 92% of the canvas unless the natural form is much smaller. Leave clearly visible transparent breathing room on every side, with especially generous space above the highest leaf/branch and at both lateral extremes. Target at least ~4% transparent top margin, ~4% on each side, and ~1% at the bottom so the production framing gate has safety buffer. Do not crop leaves, branches, fruit, flowers, stems, or roots at the canvas edge.',
    'PLANT ONLY: no people, animals, insects, birds, fish, toys, mascots, tools, signs, decorative props, ornaments, or unrelated objects. No pot, no planter, no scenery, no grass patch, no soil rectangle, no fake ground plane, no studio sweep, no baked scene shadow, no checkerboard, no text, no watermark, no labels.',
