...
test('live calibration review wires actual cutouts, not placeholders', () => {
  ...
  assert.match(review, /PHYSICAL V1/);
  ...
  assert.match(review, /Does the natural-mature Mango now have believable height and canopy relative to the real garden/);
  ...
  assert.match(review, /data-tree-scale-multiplier/);
  assert.match(review, /Form relative scale/);
...
test('RAW vs BLEND V1 is runtime CSS only and cannot claim to fix architecture', () => {
  ...
  assert.match(html, /data-composition-v2-class="RUNTIME_SCALE_SOLVABLE" disabled/);
  assert.match(html, /TREE SCALE V3/);
