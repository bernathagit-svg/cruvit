...
test('woody foliage A/B prep is two jobs, denied, and does not change production prompt or control PNG', () => {
  ...
  assert.equal(written.costPreflight, 'COST_PREFLIGHT_UNRESOLVED');
  assert.equal(written.projectedMaxSpend, 'COST_PREFLIGHT_UNRESOLVED');
  ...
  assert.equal(cost.highArm.imageOutputUsdKnown, null);
});
