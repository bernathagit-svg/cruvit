#!/usr/bin/env node
/**
 * BRANCH_STRUCTURE calibration. Default DENY. Owner-approved flags required.
 */
...
export async function main(argv = process.argv.slice(2)) {
  const parsed = parseOwnerApprovedBranchStructureCalibration(argv);
  if (parsed.ownerApprovedThisRunOnly !== true) {
    const result = executeBranchStructureCalibration();
    process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
    return result.executed ? 1 : 0;
  }
  const result = await executeBranchStructureCalibrationPaid(argv);
  const report = {
