...
const report={
  ...
  scalePolicyVersion:'in-garden-qa-scale-policy-v1.1',
...
if(report.pass!==17||report.fail!==0||report.blocked!==0||!report.allRealSavedGardenPhoto){
...
const visionRunId='cruvit-e2e-batch-b-in-garden-vision-qa-2026-09-25-v2';
const vision={
  ...
  executionPolicy:{ownerSpendApprovalRequired:true,maxCalls:17,maxRetries:0,candidateEvidenceOnly:true,productionWritesAllowed:false,registryWritesAllowed:false},
...
fs.mkdirSync(visionPath.slice(0,visionPath.lastIndexOf('/')),{recursive:true});
...
fs.mkdirSync(proposalPath.slice(0,proposalPath.lastIndexOf('/')),{recursive:true});
fs.writeFileSync(proposalPath,JSON.stringify(proposal,null,2)+'\n');

console.log(JSON.stringify({
  geometry:{pass:report.pass,fail:report.fail,blocked:report.blocked},
  visionJobs:vision.jobCount,
  proposedMaxSpendUsd:proposal.proposedMaxSpendUsd
}));
